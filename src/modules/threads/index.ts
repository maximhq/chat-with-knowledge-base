...
export class ThreadManager {
   ...
   */
  static async getThread(threadId: string, userId: string): Promise<ApiResponse<Thread>> {
    try {
   ...
   * Delete a thread
   */
  static async deleteThread(threadId: string, userId: string): Promise<ApiResponse<void>> {
    try {
      // First verify ownership
      const existingThread = await ThreadStorage.findById(threadId);
   ...
   */
  static validateThreadData(data: { title?: string }): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
